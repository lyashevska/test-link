# `olgaspackage` developer documentation
...
# install olgaspackage as an editable package
...
The second is to use `tox`, which can be installed separately (e.g. with `pip install tox`), i.e. not necessarily inside the virtual environment you use for installing `olgaspackage`, but then builds the necessary virtual environments itself by simply running:
...
cd $(mktemp -d olgaspackage.XXXXXX)
git clone git@github.com:somename/olgaspackage .
...
Visit
[https://test.pypi.org/project/olgaspackage](https://test.pypi.org/project/olgaspackage)
and verify that your package was uploaded successfully. Keep the terminal open, we'll need it later.

In a new terminal, without an activated virtual environment or an env directory:

```shell
cd $(mktemp -d olgaspackage-test.XXXXXX)
...
--extra-index-url https://pypi.org/simple olgaspackage
```
...
Don't forget to also make a [release on GitHub](https://github.com/somename/olgaspackage/releases/new).GitHub-Zenodo integration will also trigger Zenodo into making a snapshot of your repository and sticking a DOI on it.